...
const localCache =
  typeof global.localStorage !== "undefined" ? global.localStorage : "";
...
export function getAll(): Promise<DataResponse> {
  return telemetryApi.get("/api/live/data").then((response) => response.data);
}

export function getAllModule<
  T extends keyof CanData,
  P extends keyof CanData[T],
>(
  module: T,
  message: P,
...
export function getAllModuleItem<
  T extends keyof CanData,
  P extends keyof CanData[T],
  Q extends keyof CanData[T][P],
>(
  module: T,
  message: P,
  item: Q,
  ...
  return telemetryApi
    .get(`/api/${module}/${String(message)}/item/${String(item)}`, {
      params: {
        ...(where && { where: JSON.stringify(where) }),
      },
    })
...
type mitsubaModels = keyof addPrefixToObject<
  DataResponse["mitsuba"],
  "mitsuba."
>;
...
export async function createModuleItem<
  T extends keyof CanData,
  P extends keyof CanData[T],
>(module: T, message: P, data: CanData[T][P]) {
...
export function countOne(
  ...
  return telemetryApi
    .get(`/api/${telemetryType}/${message}/cnt`, {
      params: {
        ...(where && { where: JSON.stringify(where) }),
      },
    })
